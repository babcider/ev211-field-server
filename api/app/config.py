...
# 자막 발화 경계 채번(T2 · 계약 §6c v1.7). gpt-realtime-translate 스트림에는 발화 완료
# 신호가 없다(실키 조사: delta 3종 + session.created/updated 5종뿐)므로 워커가 직접 채번한다.
# **주 규칙은 종결 부호**이고 아래 시간 간격은 종결 부호 없이 끝난 발화를 회수하는 **폴백**이다.
# 실측(2026-08-16, 게이팅 ON): 번역 스트림은 발화 **안**에서도 2.61초 멈춘다(무음 게이팅으로
# 입력이 끊기면 모델이 문장 꼬리 토큰을 붙들고 있다가 늦게 뱉는다). 같은 실행에서 발화 **사이**
# 간격은 2.23초였다 — 즉 발화 안 정체가 발화 사이보다 길어 **시간 간격만으로는 가를 수 없다**.
# 그래서 폴백 임계는 모델 정체(실측 최대 2.61초)에 3배 여유를 둔 8초로 잡는다. 이 값에 걸리는
# 것은 "종결 부호 없이 말이 끊긴 채 한참 방치된 발화"뿐이라 오분할을 만들지 않는다.
AI_UTTERANCE_GAP_SECONDS = 8.0
# 종결 부호 — 카탈로그 13개 언어를 덮는다(라틴 문자권 + CJK 전각 + 힌디 danda).
