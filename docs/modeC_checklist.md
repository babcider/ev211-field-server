...
- [x] **make-before-break 55분 실경과 검증 — PASS(2026-08-17, T6)**. 로컬 도커 스택에서 67.3분 연속 송출, 55:03 경과 시점 교체 실관측(하니스 `scripts/ai_live_longrun.py`, 커밋 `f98589e`). 교체 창 무음 max 4,760ms 로 통상 발화 간 휴지 분포(직전 45초 max 4,780ms) 이내 — 교체 기인 공백 없음. `renewals` 0→1, `restarts` 420회 폴 내내 0, 자막 seq 1~3812 완전 연속, utterance_id 누락·역행 0, 예외 로그 0. `signal_events` 에 교체 시점 트랙 재발행 흔적 없음(LiveKit 발행 유지 + OpenAI WS 만 교체 직접 증거). 한계: 교체 직후 연속 입력 관측은 약 4.8초까지만(하니스가 session_age 되감김을 구간 판정에 써서 — 수정 커밋 포함), 더 두꺼운 확증이 필요하면 교체 후 2~3분 연속 입력 관측이 값싼 후속.
